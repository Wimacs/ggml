        ...
        case GGML_OP_CONV_2D_DEFORM:
            return (op->src[0]->type == GGML_TYPE_F32 || op->src[0]->type == GGML_TYPE_F16) &&
                   ...
                   ((ggml_is_contiguous(op->src[0]) &&
                     ...
                     (op->src[3] == nullptr || ggml_is_contiguous(op->src[3])) &&
                     ggml_is_contiguous(op)) ||
                    (ggml_is_contiguous_channels(op->src[0]) &&
                     ggml_is_contiguous_channels(op->src[1]) &&
                     ggml_is_contiguous_channels(op->src[2]) &&
                     (op->src[3] == nullptr || ggml_is_contiguous_channels(op->src[3])) &&
                     ggml_is_contiguous_channels(op)));
        case GGML_OP_IM2COL:
        case GGML_OP_IM2COL_3D:
        case GGML_OP_CONV_2D:
        case GGML_OP_CONV_2D_DW:
